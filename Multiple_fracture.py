...
u_inlet = 5E-4
 # m/sec = 6000 cm/min)
C_inlet = 0.22             #mass fraction #mol/m3
...
fracdata = [
            [0.012,fracwidth, 0.5, 0.01, 0.033],
            [0.012,fracwidth,0.9, 0.01, 0.018],
            [0.007,fracwidth, 0.5, 0.01, 0.006],
            [0.012,fracwidth, 0.6, 0.025, 0.02],
            [0.005,fracwidth, 0.9, 0.022, 0.007],
            [0.003,fracwidth, -0.5, 0.028, 0.033],
            [0.02,fracwidth, 0.9,  0.045, 0.017],
            [0.008,fracwidth, 0.9, 0.045, 0.033],
            [0.008,fracwidth, -0.1,  0.035, 0.0035],
            [0.009,fracwidth, 0.55, 0.052, 0.007],
            [0.012,fracwidth, 0.54, 0.061, 0.02],
            [0.013,fracwidth, 0.57,  0.065, 0.033],
            [0.005,fracwidth, 1.2,  0.073, 0.021],
            [0.005,fracwidth, 0.6,  0.083, 0.031],
            [0.025,fracwidth, 0.6,  0.084, 0.02],
            [0.01,fracwidth, 0.7,  0.073, 0.006],
            [0.01,fracwidth, 0.25,  0.086, 0.013],
            [0.003,fracwidth, -1.5, 0.085, 0.004],            
            [0.004,fracwidth, -1.3, 0.097, 0.014]
           ] 
#fracdata = [[0.00001,0.005, 0.5,0, 0]]
...
for cell in cells(mesh):
    #poro[cell] = (random.uniform(0,1)*0.2)
...
#Define boundary
def boundary1(x):
    return x[0] < tol  and x[1] > tol and x[1] < DimY - tol 

def boundary2(x):
    return (x[1] < tol  or x[1] > DimY - tol ) #or (x[0] < DOLFIN_EPS and x[1] <= 0.48) or (x[0] < DOLFIN_EPS and x[1] >= 1-0.48)
...
Right = AutoSubDomain(lambda x, on_bnd: near(x[0],0.1) and on_bnd)
...
#print ("Values on right   = ", mean(P.vector()[P_.vector() == 2]))
        ...
        for i in np.linspace(0.00035,0.039,10):
...
#
PV_b = (u_inlet * 0.04 * t)/Pore_volume
#    
plot(poro)
